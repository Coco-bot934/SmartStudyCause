...
def _compute_correct_result(
    ...
    规则：
    - 客观题（choice）：比较学生答案与正确答案 -> is_correct / score_obtained
    - 主观题（subjective/fill）：使用学生自填得分 -> is_correct = (得分 == 满分)
    """
...
def submit_answers(
    ...
    """
    # 校验试卷存在
    paper = db.query(Paper).get(body.paper_id)
    if paper is None:
